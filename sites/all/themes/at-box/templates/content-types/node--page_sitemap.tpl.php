...
$vocabulary_sitemap = taxonomy_vocabulary_machine_name_load('sitemap');
$sitemap = array();
if (!empty($vocabulary_sitemap->vid)) {
  $sitemap = taxonomy_get_tree($vocabulary_sitemap->vid, 0, null, true);
}
?>
...
<div id="main-content" class="clearfix">
  <div class="clearfix">
    <div class="sitemap-page line-no-shadow padding-bottom-60 clearfix">
      <div class="block-item first">
      
    <?php $i = 1; ?>
    <?php foreach ($sitemap as $term): ?>
      <?php if (empty($term->parents[0])): ?>
        <?php if ($term->language == $language->language):?>
        <p class="title"><?php print $term->name; ?></p>
          <?php 
            $childs = taxonomy_get_children($term->tid); 
            ?>
        <ul>
          <?php foreach ($childs as $child): ?>
            <?php if ($child->language == $language->language) :?>
            <?php $link = field_get_items('taxonomy_term', $child, 'field_sitemap_link'); ?>
              <li><a href="<?php print url($link[0]['url']); ?>">
                <?php print $child->name; ?> ›
              </a></li>
            <?php endif ?>
          <?php endforeach; //childs ?>
        </ul>
          <?php if($i == 4): //@TODO create constant variable ?>
      </div><!-- end .block-item first -->
      ...
      <div class="block-item last">
          ...
          <?php $i++;?>
        <?php endif;?>
      <?php endif; //$term->parents[0] ?>
    <?php endforeach; //$terms_sitemap ?>
      </div><!-- end .block-item last -->
    </div><!-- end .sitemap-page -->
  </div><!-- end .clearfix -->
